...
const LetakBuku = () => {
    ...
    const reloadtabel = async (rakId: any) => {
        const response = await fetch(`/admin/api/letakbuku`);
        const result = await response.json();
        const xxx = result.filter(
            (item: any) => item.rakId == Number(rakId)
        )
    ...
    const filteredItems = all ? dataletakbuku.filter(
        (item: any) => item.BukuTb.judul && item.BukuTb.judul.toLowerCase().includes(filterText.toLowerCase()),
    ...
    const columns = !all ?
        [
            ...
            {
                name: 'Jumlah',
                selector: (row: any) => row.BukuTb.stok,
                sortable: true,
            },
            {
                name: 'Action',
                cell: (row: any) => (
                    <div className="d-flex">
                        <Delete letakbukuId={row.id} reload={reload} reloadtabel={reloadtabel} rakId={rakId} />
                    </div>
                ),
            },
                ...
                <div className="col-md-12 grid-margin stretch-card">
                    <div className="card">
                        <div className="card-header">
                            <div className='row'>
                                ...
                                <div className="card-title col-md-3">
                                    <div className="row">
                                        <div className="col-md-12">
                                            <h6 className="form-label" style={{  fontWeight: 'bold' }}>Lemari</h6>
                                            <select
                                                required
                                                className="form-control"
                                                value={lemariId} onChange={handlelemari}
                                            >
                                                <option value="" disabled={!!lemariId}>Pilih Lemari</option>
                                ...
                                <div className="card-title col-md-3">
                                    <div className="row">
                                        <div className="col-md-12">
                                            <h6 className="form-label" style={{  fontWeight: 'bold' }}>Rak</h6>
                                            {lemariId === '' ?
                                                <select
                                                    required
                                                    className="form-control"
                                                    value={rakId} onChange={handlerak}
                                                >
                                                ...
                                                <select
                                                    ...
                                                    value={rakId} onChange={handlerak}
                                                >
                                                    <option value="" disabled={!!rakId}>Pilih Rak</option>
                        ...
                        <div className="card-body">
                            ...
                            />
                            <Add reload={reload} rakId={rakId} buku={databuku} dataAll={dataletakbuku} reloadtabel={reloadtabel} />
                        </div>
                    </div>
                </div>
            </div >
